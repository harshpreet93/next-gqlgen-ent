...
func (uq *UserQuery) QueryPet() *PetQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, user.PetTable, user.PetPrimaryKey...),
		)
	...
	if query := uq.withPet; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.PetPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "pet": %v`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "pet" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Pet = append(nodes[i].Edges.Pet, n)
			}
		}
	}
